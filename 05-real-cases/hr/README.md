...
> **目标**：让 AI 能回答"销售总监还招不招"、"有几个人进了 offer 阶段"

## 0. 谁在用？

| 角色 | 怎么用 | 频率 |
|------|--------|------|
| **HR 总监** | 看仪表盘（build_dashboard.py 风格） | 每天 |
| **HR 专员** | 用 Claude 问（mcp_server.py 风格）| 每天 |
| **HR 数据分析师** | 写新查询（hr-query.py 风格）| 每周 |
| **开发者（你）** | 跑 hr-query.py 验证数据 | 1 次 |

**这套系统不是给 HR 写代码用的**——是给开发者搭好，让 HR 用友好界面。
